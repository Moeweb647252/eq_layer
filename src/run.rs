...
use crate::{eq::Equalizer, settings::Settings};
use cpal::{
    StreamConfig,
    traits::{DeviceTrait, StreamTrait},
};

pub fn run(input_device: Device, output_device: Device, settings: Settings) -> Result<()> {
    let stream_config: StreamConfig = input_device.default_input_config()?.into();

    let mut eq = Equalizer::new(settings.eq_profile.clone(), stream_config.sample_rate.0);
